...
export default function ThemeToggle() {
  ...
  return (
    ...
    >
      ...
      <Sun
        className={`pointer-events-none absolute left-1 h-4 w-4 transition-opacity duration-300 ${
          isDark ? 'opacity-0' : 'opacity-100 text-amber-500'
        }`}
      />
      <Moon
        className={`pointer-events-none absolute right-1 h-4 w-4 transition-opacity duration-300 ${
          isDark ? 'opacity-100 text-indigo-300' : 'opacity-0'
        }`}
      />
      {/* knob */}
      <span
        className={`absolute top-0.5 left-0.5 h-7 w-7 rounded-full shadow-md transition-transform duration-300 ${
